(defproject tutorial "0.1.0-SNAPSHOT"
  ...
  {:server {:clean-targets ["target"]}
   :dev
   [:server
    {:npm {:package {:main "target/out/tutorial.js"
                     :scripts {:start "node target/out/tutorial.js"}}}
     :dependencies [[figwheel-sidecar "0.5.16"]]
     :cljsbuild
     {:builds {:dev
               {:source-paths ["env/dev" "src/server"]
                :figwheel     true
                :compiler     {:main          tutorial.app
                               :output-to     "target/out/tutorial.js"
    ...
    {:builds
     {:test
      {:source-paths ["env/test" "src/server" "test"]
       :compiler     {:main tutorial.app
                      :output-to     "target/test/tutorial.js"
                      :target        :nodejs
                      :optimizations :none
                      :pretty-print  true
                      :source-map    true}}}}
   ...
   :release
   {:npm {:package {:main "target/release/tutorial.js"
                    :scripts {:start "node target/release/tutorial.js"}}}
    :cljsbuild
    {:builds
     {:release
      {:source-paths ["env/prod" "src/server"]
       :compiler     {:main          tutorial.app
                      :output-to     "target/release/tutorial.js"
